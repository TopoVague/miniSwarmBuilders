...
#   Our operations on the frame come here
    ...
    #out.write(imgTmp)
    savedShotCnt=savedShotCnt+1
    
    print ('savedShotCnt is :',savedShotCnt)
    if savedShotCnt == frameSkip/2:
        
        cv2.imwrite(framePath + fileNum +'frameCapture%d.jpg'%count,imgTmp)
        count= count+1
    ...
    if (cv2.waitKey(1) & 0xFF) == ord('p'):
            cv2.imwrite(framePath +fileNum+ 'frameCapture%d.jpg'%savedShotCnt,imgTmp)
            savedShotCnt=savedShotCnt+1
