...
class Robot extends Part {
  ...
  void move() {
    ...
    int move_skips=(int)random(1, 100);    //Determines how often random rotations are applied
    move_count=move_count + 1;
    if ( ( move_count % move_skips ) == 0) { //At random intervals apply a small random rotation
      move_count=0;
      body.setTransform(body.getPosition(), a+random(-PI/40, PI/40));
    }
    Vec2 forceVector = new Vec2(force_scale*sin(a), -cos(a)*force_scale);  
    body.applyForce(forceVector.mul(random(0.8*1.5)), body.getWorldCenter());                  //Apply a force to the robot
