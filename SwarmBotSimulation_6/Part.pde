...
// A part that is being pushed around
class Part {
...
// write yourself to a space separated value file
 void logpose(PrintWriter ofile){
  ...
  void jumpRandom(){
    ...
    //only randomly place Dynamic objects
    Vec2 pos = box2d.getBodyPixelCoord(body);  
    //body.setTransform(box2d.vectorPixelsToWorld(new Vec2(random(pos.x-10,5), random(-height/2+height/5,height-100))),random(TWO_PI));
    body.setTransform(box2d.vectorPixelsToWorld(pos.x+random(-20,20), pos.y+random(-25,25)),random(TWO_PI));
 
}//jumpRandom 

  void robotJumpRandom(){
    
    //only randomly place Dynamic objects
    Vec2 pos = box2d.getBodyPixelCoord(body);  
    //body.setTransform(box2d.vectorPixelsToWorld(new Vec2(random(pos.x-10,5), random(-height/2+height/5,height-100))),random(TWO_PI));
    body.setTransform(box2d.vectorPixelsToWorld(pos.x+random(-20,20), pos.y+random(-20,20)),random(TWO_PI));
